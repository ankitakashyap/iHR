//
//  Name of the file:BloodPRessureController.m
//  Name of the App: iHR
...
//retrieve Blood pressure details of the user from the table
...
- (IBAction)segmentchange:(id)sender {
    ...
    if (self.segment.selectedSegmentIndex==0) {
        ...
        for(int i=0; i< self.conditions.count;i++){
            NSString* tmp = [self.conditions[i][1] stringByAppendingString:@"\\"];
            tmp = [tmp stringByAppendingString:self.conditions[i][2]];
            [self.condDisplay addObject:tmp];
        ...
        for(int i=0; i< self.conditions.count;i++){
            ...
            if(result==NSOrderedAscending)
                ...
                NSString* tmp = [self.conditions[i][1] stringByAppendingString:@"\\"];
                tmp = [tmp stringByAppendingString:self.conditions[i][2]];
                [self.condDisplay addObject:tmp];                NSLog(@"falls within 6month");
            }
            
            else{
                NSString* tmp = [self.conditions[i][1] stringByAppendingString:@"\\"];
                tmp = [tmp stringByAppendingString:self.conditions[i][2]];
                [self.condDisplay addObject:tmp];                NSLog(@"same day");
            }
