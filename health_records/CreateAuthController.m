...
- (IBAction)submitAuthDetails:(id)sender {
    ...
    ProfileViewController *pvc = [tabbar.viewControllers objectAtIndex:0];

    pvc.itemFirstName = self.itemFirstName;
    pvc.itemLastName = self.itemLastName;
    pvc.itemDob = self.itemDob;
    pvc.itemAge = self.itemAge;
    pvc.itemGender = self.itemGender;
    pvc.itemEmail = self.itemEmail;
    pvc.itemContactno = self.itemContactno;
    pvc.itemBloodGroup = self.itemBloodGroup;
    pvc.itemAddr = self.itemAddr;
    pvc.itemZipcode = self.itemZipcode;
    pvc.itemUsername = self.itemUsername;
    pvc.itemImagePath = self.imagePath;
