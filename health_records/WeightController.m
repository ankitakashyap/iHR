    ...
    else
    {
        ...
        if(rc == SQLITE_OK)
        {
            while (sqlite3_step(stmt) == SQLITE_ROW) //get each row in loop
            {
                ...
                self.retWeight =[NSString stringWithUTF8String:(const char *)sqlite3_column_text(stmt, 3)];
                ...
                if([self.retUsername isEqualToString:self.itemUserName]){
                    NSMutableArray *dateCond = [NSMutableArray arrayWithObjects:tdate,self.retWeight, nil];
...
- (IBAction)discardv2:(id)sender {
}

- (IBAction)addv2:(id)sender {
}

- (IBAction)refreshv2:(id)sender {
}
@end