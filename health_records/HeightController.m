//
//  Name of the file:HeightController.m
//  Name of the App: iHR
...
@implementation HeightController
@synthesize segment;
...
//Retrieve height values of the user from the height table
    ...
    else
    {
        ...
        if(rc == SQLITE_OK)
        {
            while (sqlite3_step(stmt) == SQLITE_ROW) //get each row in loop
            {
                ...
                self.retHeight =[NSString stringWithUTF8String:(const char *)sqlite3_column_text(stmt, 3)];
                
                if([self.retUsername isEqualToString:self.itemUserName]){
                    NSMutableArray *dateCond = [NSMutableArray arrayWithObjects:tdate,self.retHeight, nil];
