...
#import "BloodSugarController.h"
#import "AuthViewController.h"



@interface BloodSugarController()
...
@implementation BloodSugarController
@synthesize segment;
...
//Retrieve blood sugar details of the user from the table
    ...
    else
    {
        ...
        if(rc == SQLITE_OK)
        {
            while (sqlite3_step(stmt) == SQLITE_ROW) //get each row in loop
            {
                ...
                self.retSugar =[NSString stringWithUTF8String:(const char *)sqlite3_column_text(stmt, 3)];
                
                
                if([self.retUsername isEqualToString:self.itemUserName]){
                    NSMutableArray *dateCond = [NSMutableArray arrayWithObjects:tdate,self.retSugar, nil];
